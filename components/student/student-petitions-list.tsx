...
import { Clock, AlertCircle, CheckCircle, HourglassIcon, Trash2 } from 'lucide-react';
import { format } from 'date-fns';
import { de } from 'date-fns/locale';
import { cn } from '@/lib/utils';
import { DeletePetitionDialog } from './delete-petition-dialog';
...
export function StudentPetitionsList({ petitions }: StudentPetitionsListProps) {
  ...
  const [selectedPetition, setSelectedPetition] = useState<{ id: string; title: string } | null>(
    null
  );
  ...
  const handleDeleteClick = (petition: PetitionWithReason) => {
    setSelectedPetition({ id: petition.id, title: petition.title });
                      ...
                      <div className="space-y-1 flex-1">
                        ...
                        {petition.rejection_date && (
                          <p className="text-xs text-muted-foreground mt-2">
                            Abgelehnt am{' '}
                            {format(new Date(petition.rejection_date), 'PPP', { locale: de })}
                          </p>
                        )}
                      </div>
                    </div>
